...
{
    class SteamToolsGetLogosGameMenuItemPlugin : IGameMenuItemPlugin
    {
        SteamContext context;
        public bool SupportsMultipleGames => true;

        public string Caption => "Steam Tools / Get Logos";
            ...
            foreach (IGame game in selectedGames)
            {
                if (SteamToolsContext.IsSteamGame(game))
                {
                    ...
                    if (GameIDNumber.HasValue)
                    {
                        ...
                        if(app.extra_has_library_logo)
                        {
                            if (game.GetAllImagesWithDetails("Clear Logo").Length == 0)
                            {
                                string SavePath = game.GetNextAvailableImageFilePath(".png", "Clear Logo", null);
                                try
                                {
                                    webClient.DownloadFile($@"https://steamcdn-a.akamaihd.net/steam/apps/{GameIDNumber.Value}/logo.png?t={DateTimeOffset.UtcNow.ToUnixTimeSeconds()}", SavePath);
                                }
                                catch (Exception ex)
                                {
        ...
        public SteamToolsGetLogosGameMenuItemPlugin()
        {
